...
function Library:Main(GName)
    ...
    function TabLibrary:Notify(NText, Time)
        ...
        NotificationLogo.Image = "rbxassetid://"
    ...
    function TabLibrary:Tab(TName)
        ...
        function ItemLibrary:Label(LText, Lines)
        ...
        function ItemLibrary:Button(BName, callback)
        ...
        function ItemLibrary:Toggle(TGName, State, callback)
        ...
        function ItemLibrary:Slider(SName, MinVal, MaxVal, Inc, StartValue, callback)
        ...
        function ItemLibrary:TextBox(TBName, Text, Keep, callback)
        ...
        function ItemLibrary:Keybind(KName, Key, callback)
        ...
        function ItemLibrary:Dropdown(DName, Items, ShowItem, callback)
        ...
        function ItemLibrary:Colorpicker(CName, Color, callback)
    ...
    function TabLibrary:CreateConfigTab()
        if not isfolder("pulawat") then makefolder("pulawat") end
        if not isfolder("pulawat/" .. game.GameId) then
            makefolder("pulawat/" .. game.GameId)
        end
        local DefaultConfig = Config
        if DefaultConfig then
            writefile("pulawat/" .. game.GameId .. "/default.json",
        ...
        local NM
        local ConfigTab = TabLibrary:Tab("Settings")
        ConfigTab:Label("Main", true)
        ConfigTab:Slider("Rainbow Speed", 1,10,1,7,function(v)
            RainbowSpeed = 11-v
        end)
        ConfigTab:Label("Configs", true)
        local ConfigDropdown = ConfigTab:NewDropdown("Configs", configs, true, function(v)
            SelectedConfig = v
        end)
        ConfigDropdown:Clear()
        wait(0.2)
        for _, v in pairs(listfiles("pulawat/"..game.GameId)) do
            table.insert(configs, string.split(v,"pulawat/"..game.GameId.."\\")[2])
            ConfigDropdown:AddItem(tostring(string.split(v,"pulawat/"..game.GameId.."\\")[2]))
        end
        ConfigDropdown:Set("default.json")
        ConfigTab:Button("Load", function()
            local c = zzHttpService:JSONDecode(readfile("pulawat/".. game.GameId.."/"..string.lower(SelectedConfig)))
        ...
        ConfigTab:Button("Overwrite", function()
            if string.lower(SelectedConfig) == "default.json" then
                TabLibrary:Notify("Default.json cannot be overwriten!", 2)
            else
                writefile("pulawat/"..game.GameId .."/"..string.lower(SelectedConfig),zzHttpService:JSONEncode(Config))
            end
        end)
        ConfigTab:TextBox("Config Name","...",true, function(v) 
            NM = string.lower(v) 
        end)
        ConfigTab:Button("Create", function()
            if isfile("pulawat/"..game.GameId .."/"..string.lower(NM)..".json") or string.lower(NM) == "default" then
                TabLibrary:Notify("Config already exists!", 2)
            else
                writefile("pulawat/"..game.GameId.."/"..string.lower(NM)..".json",zzHttpService:JSONEncode(Config))
                configs = {}
                ConfigDropdown:Clear()
                wait(1)
                for _, v in pairs(listfiles("pulawat/"..game.GameId)) do
        ...
        local Credits = tostring("pulawat #6680")
        ConfigTab:Dropdown("Credits", Credits, false, function(v)
            return
        end)
